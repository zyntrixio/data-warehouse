...
schedule = Schedule(clocks=[clocks.CronClock("0 0 5 ? * 1#1 *")]) ## Runs at 5:00 first Sunday of the month

with Flow(
        name="Bink ELT Monthly Full Load"
        ...
        dbt_deps = dbt_deps_task(
            upstream_tasks=[compile_profiles_temp]
        )
