...
WITH lc_metric AS (
    SELECT *,
           'JOINS' AS category
    FROM metrics.loyalty_cards.lc__links_joins__monthly_retailer_channel
    WHERE loyalty_plan_company = 'Stonegate Group'
     ...
     txn_metrics AS (
         SELECT *,
                'SPEND' AS category
         FROM metrics.transactions.trans__trans__monthly_retailer_channel
         WHERE loyalty_plan_company = 'Stonegate Group'
           AND channel = 'LLOYDS'),
     ...
     txn_avg AS (
         SELECT *,
                'SPEND' AS category
         FROM metrics.transactions.trans__avg__monthly_retailer_channel
         WHERE loyalty_plan_company = 'Stonegate Group'
           AND channel = 'LLOYDS'),
     ...
     user_metrics AS (
         SELECT *,
                'USERS' AS category
         FROM metrics.users.user__transactions__monthly_retailer_channel
         WHERE loyalty_plan_company = 'Stonegate Group'
           AND channel = 'LLOYDS'),
     ...
     pll_metrics AS (
         SELECT *,
                'JOINS' AS category
         FROM metrics.loyalty_cards.lc__pll__monthly_retailer_channel
         WHERE loyalty_plan_company = 'Stonegate Group'
           AND channel = 'LLOYDS'),
     ...
     combine_all AS (
         ...
         SELECT date,
                ...
                loyalty_plan_name,
                loyalty_plan_company,
                NULL AS lc312__successful_loyalty_cards__monthly_channel_brand_retailer__count,
                NULL AS lc324__successful_loyalty_card_links__monthly_channel_brand_retailer__dcount_user,
                NULL AS lc371__successful_loyalty_card_joins__monthly_channel_brand_retailer__csum,
                NULL AS lc385__successful_loyalty_card_join_mrkt_opt_in_percentage__monthly_channel_brand_retailer__percentage,
