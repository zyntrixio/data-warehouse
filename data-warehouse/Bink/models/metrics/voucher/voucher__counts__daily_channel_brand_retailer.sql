...
with
    ...
    voucher_metrics as (
        select
            ...
            coalesce(
                count(distinct case when state = 'REDEEMED' then voucher_code end), 0
        ...
        group by d.date, v.channel, v.brand, v.loyalty_plan_company, v.loyalty_plan_name
    ),
    rename as (
        select
            date,
            ...
            sum(daily_redeemed_vouchers) over (
                partition by loyalty_plan_company, brand order by date asc
